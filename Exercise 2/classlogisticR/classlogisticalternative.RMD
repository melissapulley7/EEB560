...
For binomial data, we use the **LOGISTIC REGRESSION**
...
```{r}
orchiddata <- readRDS("~/projects/EEB411_560/orchid.RDS")
```
...
However, these linear predictors are for the transformed response variable (logit of odds). To loo at the actual value predictors, we use `predict()`:
...
Finally... let's look at the ocurrence probability in each habitat type:
