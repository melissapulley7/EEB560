...
\answerbox{A5. NIB is skewed left. VCR has the largest IQR. NIB's 50th percentile is higher than VCR's 25th percentile.}
...
```{r}
mean_ls = mean(length_Scelloporus)
sd_ls = sd(length_Scelloporus)

z=(length_Scelloporus-mean_ls)/sd_ls
z
```

```{=tex}
\answerbox{A8. The z-value for each observation is are 

\vspace{1ex}

\begin{tabular}{|l|l|}
\hline
Length of Scelloporus & Z-value \\ \hline
49.79345 & 1.0910741 \\ \hline
48.27289  &  0.8228193 \\ \hline
47.01204 & 0.6003821 \\ \hline
50.04918 & 1.1361896 \\ \hline
33.23075 & -1.8308916 \\ \hline
36.95524 & -1.1738229 \\ \hline
45.49277 & 0.3323549 \\ \hline
44.33337 & 0.1278153 \\ \hline
40.92777 & -0.4729953 \\ \hline
40.02123 & -0.6329257 \\ \hline
\end{tabular}
}
```
```{r echo=FALSE, eval=FALSE}
...
```{=tex}
\answerbox{A9. We have the following hypotheses:

$H_0$: The means of the original teporingos and teporingos with a diet are equal.

$H_1$: The means of the original teporingos and teporingos with a diet are equal.

Since the p-value $2.135\times 10^{-6} \leq 0.05$, we can reject the null hypothesis, and we have support that the means of the two teporingos populations are different.
}
```
## Linear models
...
```{=tex}
\answerbox{A10. The proposed model is:
$$y=1.3276 + 0.5465*(\text{Petal.Width}) + 0.4537*(\text{Speiciesversicolor})+ 2.9131 (\text{Speciesvirinica}) + 1.3228 (\text{Petal.Width}) \cdot (\text{Speiciesversicolor}) + 0.1008 (\text{Petal.Width})\cdot (\text{Speiciesversicolor}) + \epsilon $$

We can also note from our summary that since the median residual is near 0, we will say that the error term is zero.

We can find the predicted petal length is 2.4206.
}
```
```{r}
petal_length= 1.3276 + 0.5465*2 + 0.4537*0+ 2.9131*0 + 1.3228*2*0 + 0.1008*2*0
petal_length
```
...
```{=tex}
\answerbox{A11. (1) I have enjoyed learning new coding techniques and expanding the range of tools I can use by practicing to use .quarto and .rmd files.
(2) I have also enjoyed learning refreshing my memory on frenquentist statistics methodology, whereas I have largely used Bayesian Statistics over the past couple years.
}
```